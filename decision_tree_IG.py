...
#            score += prob*prob
            if prob > 0 :
                score -= float(prob * math.log(prob, 2))
#            print(score)
        # update entropy value
        entropy += (1-score)*size/total
...
def get_split(dataset):
    ...
    return best_feature,best_value,best_right,best_left,entropy
...
class Node(object):    
    ...
    def build_tree(self):
        ...
        if self.current_depth > max_depth or label_prob >= 1:
            ...
            self.label = None
            # split the node
            self.feature_index,self.value,right_dataset,left_dataset,best_ent = get_split(self.dataset)
            print('[{}], {} samples, X{}=={}, ent={}'.
                  format(self.current_depth,len(self.dataset),self.feature_index,self.value,best_ent))                
            # go right
            print('-->')
